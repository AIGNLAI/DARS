#!/usr/bin/env python3
"""EmbedLLM model-embedding router for single-shot and DARS supervision.

The implementation follows the DARS paper appendix. It learns a query
projection and one compact embedding per candidate model, then predicts
query-model performance and cost.
"""
...
class EmbedLLMNet(nn.Module):
    ...
    def forward(
        ...
        model_ids = torch.arange(
            self.model_embed.num_embeddings, device=query_features.device
        )
...
class EmbedLLMRouter:
    def __init__(
        self, model: EmbedLLMNet, config: EmbedLLMConfig, stats: dict[str, Any]
    ) -> None:
        self.model = model
        self.config = config
        self.stats = stats
...
def embed_targets(
    ...
    risk_score = np.clip(score - float(risk_beta) * score_std, 0.0, 1.0).astype(
        np.float32
    )
    return score, cost, risk_score


def train_embed_llm_model(
    x_train: Any,
    ...
    if score.shape != cost.shape or score.shape[0] != features.shape[0]:
        raise ValueError(
            "Target score/cost matrices must align with training features."
        )
...
def run_experiment(
    ...
    for dataset in datasets:
        ...
        if mode in {"distribution", "both"}:
            ...
            router_name = "embed_llm_distribution"
            predictions, model_predictions = predict_embed_llm(
                router,
                ds_x_test,
                models,
                risk_beta=risk_beta,
        ...
        if mode in {"single-point", "both"}:
            for run in range(config.single_point_runs):
                ...
                router_name = f"embed_llm_single_point_{run:03d}"
                predictions, model_predictions = predict_embed_llm(
                    router,
                    ds_x_test,
                    models,
    ...
    if not summaries:
        raise ValueError(
            "No EmbedLLM routers were trained. Check dataset names and input files."
        )
    ...
    summary_df.to_csv(output_dir / "embed_llm_summary.csv", index=False)
    summarize_runs(summary_df).to_csv(
        output_dir / "embed_llm_summary_by_mode.csv", index=False
    )
    baselines.to_csv(output_dir / "embed_llm_test_baselines.csv", index=False)
    pd.concat(prediction_frames, ignore_index=True).to_csv(
        output_dir / "embed_llm_predictions.csv", index=False
    )
    pd.concat(model_prediction_frames, ignore_index=True).to_csv(
        output_dir / "embed_llm_model_predictions.csv", index=False
    )
    pd.concat(evaluation_frames, ignore_index=True).to_csv(
        output_dir / "embed_llm_evaluation_detail.csv", index=False
    )
    return summary_df
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=project_dir / "outputs" / "embed_llm",
    )
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.weight_decay < 0 or args.alpha < 0 or args.cost_loss_weight < 0:
        raise ValueError(
            "--weight-decay, --alpha, and --cost-loss-weight must be non-negative."
        )
