#!/usr/bin/env python3
"""AvengersPro-style cluster router for distribution-aware LLM routing.

This adapts the LAMDA-ORBIT AvengersPro reference to the local scored-generation
data. It clusters training queries, stores per-cluster model score/cost/risk
statistics, and routes test queries by averaging over the nearest clusters.
"""
...
class AvengersProRouter:
    ...
    def fit(self) -> None:
        ...
        for cluster in range(self.n_clusters):
            ...
            if mask.any():
                self.cluster_score[cluster] = np.nanmean(self.target_score[mask], axis=0)
...
def run_experiment(
    ...
    for dataset in datasets:
        ...
        if mode in {"distribution", "both"}:
            ...
            router = AvengersProRouter(ds_x_train, target_score, target_cost, target_std, config)
            router.fit()
            predictions, model_predictions = predict_avengerspro(
                router,
                ds_x_test,
                models,
                risk_beta,
                "avengerspro_distribution",
                ds_test_meta,
                config,
            )
        ...
        if mode in {"single-point", "both"}:
            for run in range(config.single_point_runs):
                ...
                run_config = AvengersProConfig(
                    **{**config.__dict__, "seed": config.seed + run}
                )
                router = AvengersProRouter(
                ...
                predictions, model_predictions = predict_avengerspro(
                    ...
                    models,
                    0.0,
                    f"avengerspro_single_point_{run:03d}",
                    ds_test_meta,
                    config,
                )
                summary, detail = evaluate_predictions(predictions, rewrite_qtable, decoding_qtable)
    ...
    if not summaries:
        raise ValueError("No AvengersPro routers were trained. Check dataset names and input files.")
    ...
    summary_df.to_csv(output_dir / "avengerspro_summary.csv", index=False)
    summarize_runs(summary_df).to_csv(output_dir / "avengerspro_summary_by_mode.csv", index=False)
    baselines.to_csv(output_dir / "avengerspro_test_baselines.csv", index=False)
    pd.concat(prediction_frames, ignore_index=True).to_csv(
        output_dir / "avengerspro_predictions.csv", index=False
    )
    pd.concat(model_prediction_frames, ignore_index=True).to_csv(
        output_dir / "avengerspro_model_predictions.csv", index=False
    )
    pd.concat(evaluation_frames, ignore_index=True).to_csv(
        output_dir / "avengerspro_evaluation_detail.csv", index=False
    )
    return summary_df
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=project_dir / "analysis_outputs_avengerspro"
    )
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.n_clusters <= 0 or args.multi_cluster <= 0 or args.max_iter <= 0:
        raise ValueError("--n-clusters, --multi-cluster, and --max-iter must be positive.")
