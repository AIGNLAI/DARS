#!/usr/bin/env python3
"""kNNRouter-style distribution-aware LLM router.

The local data already contains scored query/model observations. This script
follows ``mlp.py`` for data loading, feature extraction, test splitting, and
reporting, while adapting the LAMDA-ORBIT kNNRouter idea: predict each model's
score/cost from inverse-distance weighted nearest training queries.
"""
...
class KNNRouter:
    def __init__(
        ...
        self.x_train = torch.from_numpy(np.asarray(to_dense_array(x_train), dtype=np.float32)).to(
            self.device
        )
        self.target_score = torch.from_numpy(np.asarray(target_score, dtype=np.float32)).to(
            self.device
        )
        self.target_cost = torch.from_numpy(np.asarray(target_cost, dtype=np.float32)).to(
            self.device
        )
    ...
    def predict(self, x_test: Any) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        x = torch.from_numpy(np.asarray(to_dense_array(x_test), dtype=np.float32)).to(self.device)
        ...
        with torch.no_grad():
            for start in range(0, x.shape[0], self.config.batch_size):
                ...
                score_batches.append((weights.unsqueeze(-1) * self.target_score[indices]).sum(dim=1))
                cost_batches.append((weights.unsqueeze(-1) * self.target_cost[indices]).sum(dim=1))
                std_batches.append((weights.unsqueeze(-1) * self.target_std[indices]).sum(dim=1))
...
def run_experiment(
    ...
    for dataset in datasets:
        ...
        if mode in {"distribution", "both"}:
            ...
            router = KNNRouter(ds_x_train, target_score, target_cost, target_std, config)
            predictions, model_predictions = predict_knn(
                router,
                ds_x_test,
                models,
                risk_beta=risk_beta,
                router_name="knn_distribution",
        ...
        if mode in {"single-point", "both"}:
            for run in range(config.single_point_runs):
                ...
                predictions, model_predictions = predict_knn(
                    ...
                    models,
                    risk_beta=0.0,
                    router_name=f"knn_single_point_{run:03d}",
                    test_meta=ds_test_meta,
                    config=config,
                )
                summary, detail = evaluate_predictions(predictions, rewrite_qtable, decoding_qtable)
    ...
    if not summaries:
        raise ValueError("No kNN routers were evaluated. Check dataset names and input files.")
    ...
    summary_df.to_csv(output_dir / "knn_summary.csv", index=False)
    summarize_runs(summary_df).to_csv(output_dir / "knn_summary_by_mode.csv", index=False)
    baselines.to_csv(output_dir / "knn_test_baselines.csv", index=False)
    pd.concat(prediction_frames, ignore_index=True).to_csv(
        output_dir / "knn_predictions.csv", index=False
    )
    pd.concat(model_prediction_frames, ignore_index=True).to_csv(
        output_dir / "knn_model_predictions.csv", index=False
    )
    pd.concat(evaluation_frames, ignore_index=True).to_csv(
        output_dir / "knn_evaluation_detail.csv", index=False
    )
    return summary_df
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=project_dir / "analysis_outputs_knn")
...
def main() -> None:
    ...
    print(f"\nSaved kNN outputs to {args.output_dir}")
