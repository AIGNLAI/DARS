#!/usr/bin/env python3
"""RM-Softmax router for single-shot and DARS supervision.

This script follows ``methods/mlp.py`` for local data loading, text encoding,
test splitting, and score reporting. RM-Softmax learns model choices from
augmented ``(query embedding, lambda)`` examples by minimizing expected regret
over observed model utility, as described in the paper appendix.
"""
...
class RouterNet(nn.Module):
    ...
    def __init__(
        self, input_dim: int, hidden_layers: Sequence[int], output_dim: int
    ) -> None:
        super().__init__()
...
def get_risk_beta(dataset: str, config: RMSoftmaxConfig) -> float:
...
def build_augmented_training(
    ...
    return (
        ...
        labels,
        utility.reshape(-1, utility.shape[2]).astype(np.float32),
    )
...
def train_rm_softmax(
    features: Any,
    performance: np.ndarray,
    cost: np.ndarray,
    seed: int,
    config: RMSoftmaxConfig,
    ...
    x_aug, labels, utilities = build_augmented_training(
        features_dense, performance, cost, lambdas
    )
    ...
    for _ in range(config.epochs):
        ...
        for start in range(0, n_samples, config.batch_size):
            ...
            if config.loss_name == "classification":
                loss = ce_loss(logits, label_tensor[index])
            elif config.loss_name == "softmax":
                ...
                raise ValueError(f"Unsupported RM loss: {config.loss_name}")
...
def predict_choices(
    ...
    features_dense = np.asarray(to_dense_array(features), dtype=np.float32)
    lambda_column = np.full(
        (features_dense.shape[0], 1), float(lambda_value), dtype=np.float32
    )
...
def run_experiment(
    ...
    for dataset in datasets:
        ...
        if mode in {"distribution", "both"}:
            ...
            _, target_cost, target_performance = build_rm_targets(
                target, ds_train_meta, models, risk_beta
            )
            model, stats = train_rm_softmax(
                ds_x_train,
                target_performance,
                target_cost,
                seed=config.seed,
            ...
            router_name = f"rm_{config.loss_name}_distribution"
        ...
        if mode in {"single-point", "both"}:
            for run in range(config.single_point_runs):
                ...
                router_name = f"rm_{config.loss_name}_single_point_{run:03d}"
                predictions, probabilities = predict_choices(
                    model,
                    ds_x_test,
                    models,
    ...
    if not summaries:
        raise ValueError(
            "No RM-Softmax routers were trained. Check dataset names and input files."
        )
    ...
    summary_df.to_csv(output_dir / "rm_softmax_summary.csv", index=False)
    summarize_runs(summary_df).to_csv(
        output_dir / "rm_softmax_summary_by_mode.csv", index=False
    )
    baselines.to_csv(output_dir / "rm_softmax_test_baselines.csv", index=False)
    pd.concat(prediction_frames, ignore_index=True).to_csv(
        output_dir / "rm_softmax_predictions.csv", index=False
    )
    pd.concat(probability_frames, ignore_index=True).to_csv(
        output_dir / "rm_softmax_model_probabilities.csv", index=False
    )
    pd.concat(evaluation_frames, ignore_index=True).to_csv(
        output_dir / "rm_softmax_evaluation_detail.csv", index=False
    )
    return summary_df
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=project_dir / "outputs" / "rm_softmax",
    )
    ...
    parser.add_argument(
        ...
        default="softmax",
        help="Training objective. 'softmax' reproduces the paper's RM-Softmax baseline.",
    )
    parser.add_argument(
        "--feature-backend",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help=f"Override distribution risk penalty beta (paper default: {DEFAULT_RISK_BETA}).",
    )
...
def main() -> None:
    ...
    print(f"\nSaved RM-Softmax router outputs to {args.output_dir}")
